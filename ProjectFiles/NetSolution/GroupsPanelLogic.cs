...
public class GroupsPanelLogic : QPlatform.NetLogic.BaseNetLogic
{
    ...
    private void BuildUIGroups()
    {
        ...
        foreach (var group in groups.Children)
        {
            if (editable.Value)
            {
                var groupCheckBox = InformationModel.MakeObject<Panel>(group.BrowseName, demo_collaboration_flow.ObjectTypes.GroupCheckbox);
            ...
            {
                var groupLabel = InformationModel.MakeObject<Panel>(group.BrowseName, demo_collaboration_flow.ObjectTypes.GroupLabel);
